defmodule Rockelivery.Orders.Create do
  ...
  alias Rockelivery.{Repo, Item, Error, Order}
  ...
  defp fetch_items(%{"items" => items_params}) do
    items_id = Enum.map(items_params, fn item -> item["id"] end)
    query = from item in Item, where: item.id in ^items_id

    query
    |> Repo.all()
    |> validate_and_multiply_items(items_id, items_params)
  end
  ...
  defp multiply_items(true, _items, _items_paramas), do: {:error, "Invalid ids"}
